<div>
    <x-card shadow>
        <x-steps wire:model="step" steps-color="step-primary" class="mb-8" stepper-classes="w-full">
            ...
            <x-step :step="4" :text="__('Queue & Scheduler')">
                <div class="divider"></div>
                <p class="text-sm opacity-70">
                    {{ __('Previews and thumbnails are generated by background jobs. A queue worker processes them and a scheduled task takes care of recurring maintenance.') }}
                </p>

                @unless($useSyncQueue)
                    <div class="mt-6">
                        <h3 class="font-semibold">{{ __('1. Queue worker') }}</h3>
                        <p class="text-xs opacity-70 mb-3">
                            {{ __('Keep a worker running so previews are generated in the background. Adjust the user and the PHP binary path to match your server.') }}
                        </p>

                        <p class="text-xs font-semibold opacity-70 mb-1">{{ __('Command') }}</p>
                        <pre class="bg-base-200 rounded-box p-3 overflow-x-auto text-xs mb-4"><code>{{ $this->queueWorkerCommand }}</code></pre>

                        <p class="text-xs font-semibold opacity-70 mb-1">{{ __('Option A — systemd') }}
                            <span class="opacity-70">(/etc/systemd/system/xbackbone-worker.service)</span></p>
                        <pre class="bg-base-200 rounded-box p-3 overflow-x-auto text-xs mb-1"><code>{{ $this->systemdUnit }}</code></pre>
                        <pre class="bg-base-200 rounded-box p-3 overflow-x-auto text-xs mb-4"><code>sudo systemctl enable --now xbackbone-worker</code></pre>

                        <p class="text-xs font-semibold opacity-70 mb-1">{{ __('Option B — Supervisor') }}
                            <span class="opacity-70">(/etc/supervisor/conf.d/xbackbone-worker.conf)</span></p>
                        <pre class="bg-base-200 rounded-box p-3 overflow-x-auto text-xs mb-1"><code>{{ $this->supervisorConfig }}</code></pre>
                        <pre class="bg-base-200 rounded-box p-3 overflow-x-auto text-xs"><code>sudo supervisorctl reread && sudo supervisorctl update</code></pre>
                    </div>
                @endunless

                <div class="mt-6">
                    <h3 class="font-semibold">{{ __('2. Scheduler (cron)') }}</h3>
                    <p class="text-xs opacity-70 mb-3">
                        {{ __('Add this line to the crontab of the user that runs the application to enable recurring maintenance tasks.') }}
                    </p>
                    <pre class="bg-base-200 rounded-box p-3 overflow-x-auto text-xs"><code>{{ $this->schedulerCronLine }}</code></pre>
                </div>

                <div class="mt-6">
                    <x-toggle :label="__('I can\'t set up a queue worker (process jobs synchronously)')"
                              :hint="__('Use this only if you cannot run a worker on this server.')"
                              wire:model.live="useSyncQueue"/>

                    @if($useSyncQueue)
                        <x-alert class="mt-4 alert-warning" icon="o-exclamation-triangle"
                                 :title="__('Synchronous processing enabled')"
                                 :description="__('Previews will be generated during the upload request instead of in the background. Uploads will be slower, and large files or videos may fail to generate a preview if the request times out. Configure a queue worker as soon as you can.')"/>
                    @endif
                </div>
            </x-step>

            {{-- Step 5: admin --}}
            <x-step :step="5" :text="__('Administrator')">
                <div class="divider"></div>
